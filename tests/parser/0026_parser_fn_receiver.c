...
OAK_TEST_DECL(ParseFnReceiver)
{
  ...
  */
  ...
  const oak_ast_node_t* stmt_lhs = oak_test_ast_child(stmt, 0);
  OAK_CHECK_NODE_KIND(stmt_lhs, OAK_NODE_KIND_IDENT);
  OAK_CHECK_TOKEN_STR(stmt_lhs, "x");

  const oak_ast_node_t* stmt_rhs = oak_test_ast_child(stmt, 1);
