...
OAK_TEST_DECL(ParseWhileBreakContinue)
{
  ...
  */
  ...
  OAK_CHECK_NODE_KIND(cond, OAK_NODE_BINARY_LESS);
  ...
  OAK_CHECK_NODE_KIND(if_stmt, OAK_NODE_STMT_IF);

  const struct oak_ast_node_t* if_body = oak_test_ast_child(if_stmt, 1);
  OAK_CHECK_NODE_KIND(if_body, OAK_NODE_BLOCK);
  const struct oak_ast_node_t* break_stmt = oak_test_ast_child(if_body, 0);
  OAK_CHECK_NODE_KIND(break_stmt, OAK_NODE_STMT_BREAK);
  ...
  OAK_CHECK_NODE_KIND(cont_stmt, OAK_NODE_STMT_CONTINUE);
