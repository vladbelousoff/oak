...
{
  struct oak_lexer_result_t* lexer = OAK_LEX("1 + foo(2) * 3;");
  ...
  */
  ...
  const struct oak_ast_node_t* call = mul->lhs;
  ...
  OAK_CHECK_NODE_KIND(callee, OAK_NODE_IDENT);
  OAK_CHECK_TOKEN_STR(callee, "foo");

  const struct oak_ast_node_t* arg = oak_test_ast_child(call, 1);
  OAK_CHECK_NODE_KIND(arg, OAK_NODE_FN_CALL_ARG);
  OAK_CHECK_CHILD_COUNT(arg, 1);
  OAK_CHECK_NODE_KIND(oak_test_ast_child(arg, 0), OAK_NODE_INT);
