...
OAK_TEST_DECL(ParseIndexAccess)
{
  oak_lexer_result_t* lexer =
      oak_lexer_tokenize("arr[0]; obj.items[i]; matrix[0][1];");
  ...
  /*
     Expected shape:
       PROGRAM
         ...
         STMT_EXPR
           INDEX_ACCESS [MEMBER_ACCESS [IDENT("obj"), IDENT("items")], IDENT("i")]
         STMT_EXPR
           INDEX_ACCESS [INDEX_ACCESS [IDENT("matrix"), INT(0)], INT(1)]
  */

  const oak_ast_node_t* stmt0 = oak_test_ast_child(root, 0);
  OAK_CHECK_NODE_KIND(stmt0, OAK_NODE_KIND_STMT_EXPR);
  const oak_ast_node_t* idx0 = oak_test_ast_child(stmt0, 0);
  ...
  const oak_ast_node_t* idx1 = oak_test_ast_child(stmt1, 0);
  OAK_CHECK_NODE_KIND(idx1, OAK_NODE_KIND_INDEX_ACCESS);
  OAK_CHECK_CHILD_COUNT(idx1, 2);
  const oak_ast_node_t* member = oak_test_ast_child(idx1, 0);
  ...
  const oak_ast_node_t* stmt2 = oak_test_ast_child(root, 2);
  OAK_CHECK_NODE_KIND(stmt2, OAK_NODE_KIND_STMT_EXPR);
  const oak_ast_node_t* outer = oak_test_ast_child(stmt2, 0);
  OAK_CHECK_NODE_KIND(outer, OAK_NODE_KIND_INDEX_ACCESS);
  const oak_ast_node_t* inner = oak_test_ast_child(outer, 0);
