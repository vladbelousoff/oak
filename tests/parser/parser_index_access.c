...
OAK_TEST_DECL(ParseIndexAccess)
{
  ...
  */
  ...
  OAK_CHECK_NODE_KIND(idx0, OAK_NODE_INDEX_ACCESS);
  OAK_CHECK_CHILD_COUNT(idx0, 2);
  OAK_CHECK_NODE_KIND(oak_test_ast_child(idx0, 0), OAK_NODE_IDENT);
  OAK_CHECK_TOKEN_STR(oak_test_ast_child(idx0, 0), "arr");
  OAK_CHECK_NODE_KIND(oak_test_ast_child(idx0, 1), OAK_NODE_INT);
