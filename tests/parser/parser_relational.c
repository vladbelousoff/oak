...
check_binary_relational(const char* source, const enum oak_node_kind_t kind)
{
  ...
  */
  ...
  OAK_CHECK_NODE_KIND(binary, kind);
...
OAK_TEST_DECL(ParseRelational)
{
  static const struct
  {
    ...
    { "a < b;", OAK_NODE_BINARY_LESS },
    { "a <= b;", OAK_NODE_BINARY_LESS_EQ },
    { "a > b;", OAK_NODE_BINARY_GREATER },
    { "a >= b;", OAK_NODE_BINARY_GREATER_EQ },
  };

  for (usize i = 0; i < oak_count_of(cases); ++i)
  {
    const enum oak_test_status_t status =
        check_binary_relational(cases[i].source, cases[i].kind);
