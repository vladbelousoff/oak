...
OAK_TEST_DECL(ParseLvalueAssign)
{
  oak_lexer_result_t* lexer = oak_lexer_tokenize(
      "obj.x = 1; arr[0] = 2; obj.x += 3; arr[i] -= 4;");
  ...
  */
  ...
  const oak_ast_node_t* s0_lhs = oak_test_ast_child(stmt0, 0);
  ...
  const oak_ast_node_t* s1_lhs = oak_test_ast_child(stmt1, 0);
  ...
  const oak_ast_node_t* s2_lhs = oak_test_ast_child(stmt2, 0);
  ...
  const oak_ast_node_t* stmt3 = oak_test_ast_child(root, 3);
  OAK_CHECK_NODE_KIND(stmt3, OAK_NODE_KIND_STMT_SUB_ASSIGN);
  OAK_CHECK_CHILD_COUNT(stmt3, 2);
  const oak_ast_node_t* s3_lhs = oak_test_ast_child(stmt3, 0);
