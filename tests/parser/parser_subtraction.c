...
{
  struct oak_lexer_result_t* lexer = OAK_LEX("10 - 3;");
  ...
  */
  ...
  OAK_CHECK_NODE_KIND(sub, OAK_NODE_BINARY_SUB);
  OAK_CHECK_NODE_KIND(sub->lhs, OAK_NODE_INT);
  OAK_CHECK_INT_VAL(sub->lhs, 10);
  OAK_CHECK_NODE_KIND(sub->rhs, OAK_NODE_INT);
