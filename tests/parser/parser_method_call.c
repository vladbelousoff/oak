...
{
  oak_lexer_result_t* lexer = oak_lexer_tokenize("obj.method(x = 1, y = 2);");
  ...
  */
  ...
  OAK_CHECK_NODE_KIND(callee, OAK_NODE_KIND_MEMBER_ACCESS);
  OAK_CHECK_CHILD_COUNT(callee, 2);

  const oak_ast_node_t* receiver = oak_test_ast_child(callee, 0);
  OAK_CHECK_NODE_KIND(receiver, OAK_NODE_KIND_IDENT);
  OAK_CHECK_TOKEN_STR(receiver, "obj");

  const oak_ast_node_t* method = oak_test_ast_child(callee, 1);
  OAK_CHECK_NODE_KIND(method, OAK_NODE_KIND_IDENT);
