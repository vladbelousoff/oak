...
{
  OAK_CHECK(sizeof(oak_value_t) == 8);
...
{
  const oak_value_t v = OAK_VALUE_NONE;
  OAK_CHECK(oak_is_none(v));
...
OAK_TEST_DECL(ObjectPointerRoundTrip)
{
  ...
  oak_obj_string_t* str = oak_string_new(&allocator, "hello");

  const oak_value_t v = OAK_VALUE_OBJ(str);
  OAK_CHECK(oak_is_obj(v));
  OAK_CHECK(oak_is_string(v));
  OAK_CHECK(oak_val_obj_ptr(v) == (oak_obj_t*)str);
...
OAK_TEST_DECL(WeakPointerRoundTrip)
{
  oak_allocator_t allocator;
  oak_system_allocator_init(&allocator);
  oak_obj_string_t* str = oak_string_new(&allocator, "weak");
  ...
  OAK_CHECK(oak_val_obj_ptr(weak) == (oak_obj_t*)str);
...
OAK_TEST_DECL(WeakExpiresWhenObjectDies)
{
  oak_allocator_t allocator;
  oak_system_allocator_init(&allocator);
  oak_obj_string_t* str = oak_string_new(&allocator, "gone");
...
OAK_TEST_DECL(WeakStaysExpiredAfterSlotReuse)
{
  oak_allocator_t allocator;
  oak_system_allocator_init(&allocator);
  oak_obj_string_t* first = oak_string_new(&allocator, "first");
  ...
  oak_obj_string_t* second = oak_string_new(&allocator, "second");
...
OAK_TEST_DECL(PerVmTableIsolationAndRecycle)
{
  ...
  oak_obj_string_t* scoped = oak_vm_string_new(&vm, "scoped");

  oak_obj_string_t* shared = oak_string_new(&allocator, "shared");
  OAK_CHECK(scoped->obj.table_id == table);
  OAK_CHECK(shared->obj.table_id == 0);

  const oak_value_t v = OAK_VALUE_OBJ(scoped);
  OAK_CHECK(oak_value_obj_table(v) == table);
  OAK_CHECK(oak_is_string(v));

  const oak_value_t weak = oak_value_weaken(v);
  ...
  oak_obj_string_t* reborn = oak_vm_string_new(&next_vm, "reborn");
  OAK_CHECK(oak_is_expired_weak(weak));
...
OAK_TEST_DECL(TableRegistryRecyclesUnderChurn)
{
  ...
  for (int i = 0; i < 100; ++i)
  {
    ...
    oak_obj_string_t* str = oak_vm_string_new(&vm, "churn");
    oak_obj_decref((oak_obj_t*)str);
    oak_vm_free(&vm);
  }
  return OAK_TEST_OK;
...
OAK_TEST_DECL(ObjectRefcopyCompatibilityRejectsOtherVmTables)
{
  ...
  oak_obj_string_t* vm_owned = oak_vm_string_new(&vm_a, "vm");

  oak_obj_string_t* shared = oak_string_new(&allocator, "shared");

  const oak_value_t vm_value = OAK_VALUE_OBJ(vm_owned);
  const oak_value_t shared_value = OAK_VALUE_OBJ(shared);
  ...
  oak_obj_array_t* array_b = oak_vm_array_new(&vm_b);
  oak_obj_map_t* map_b = oak_vm_map_new(&vm_b);
...
OAK_TEST_DECL(VmCallRejectsValuesFromAnotherVm)
{
  ...
  oak_obj_string_t* owned_by_a = oak_vm_string_new(&vm_a, "vm-a");
  const oak_value_t a_value = OAK_VALUE_OBJ(owned_by_a);
