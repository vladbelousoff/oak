...
/* Reading a number field from a struct compiles and runs. */
OAK_TEST_DECL(FieldAccessNumberOk)
{
  return expect_ok("type Point struct { x : number; y : number; }\n"
                   "let p = new Point { x : 3, y : 4 };\n"
                   "print(p.x);\n"
                   "print(p.y);\n");
}

/* Reading a string field from a struct compiles and runs. */
OAK_TEST_DECL(FieldAccessStringOk)
{
  return expect_ok("type Named struct { label : string; }\n"
                   "let n = new Named { label : 'hello' };\n"
                   "print(n.label);\n");
}

/* A field whose type is another struct can be accessed and its own fields
 * can be read in a chained expression. */
OAK_TEST_DECL(FieldAccessChainedOk)
{
  return expect_ok("type Inner struct { v : number; }\n"
...
/* Accessing a field that does not exist on the struct is a compile error. */
...
/* A struct field whose type is another struct propagates correctly, so that
 * it can be passed to a function expecting that struct type. */
...
/* Passing a struct-typed field to a function expecting a different struct
 * is a compile error even when the structs share the same field layout. */
OAK_TEST_DECL(StructFieldTypeToWrongFnParamFails)
{
  return expect_compile_error(
      "type A struct { v : number; }\n"
      "type B struct { v : number; }\n"
      "type Outer struct { a : A; }\n"
...
/* Assigning a number to a number field on a mutable struct compiles and
 * runs. */
...
/* Assigning a string to a string field on a mutable struct compiles and
 * runs. */
...
/* Chained field assignment through a mutable struct compiles and runs. */
...
{
  return expect_compile_error("type Named struct { label : string; }\n"
                              "let mut n = new Named { label : 'ok' };\n"
                              "n.label = 42;\n");
}

/* Assigning a struct of the wrong type to a struct-typed field is a
 * compile error. */
...
/* =========================================================================
 * Field assignment on non-struct receiver
 * ========================================================================= */
...
int main(const int argc, char* argv[])
{
  ...
  static struct oak_test_t tests[] = {
    ...
    /* field access/assignment on non-struct */
