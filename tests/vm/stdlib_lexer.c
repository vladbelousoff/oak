...
OAK_TEST_DECL(StdlibLexerTokenizeFieldAccess)
{
...
OAK_TEST_DECL(StdlibLexerArrayReturnType)
{
  const char* source = "let toks = OakLexer.tokenize('x');\n"
                       "let n = toks.size();\n"
                       "print(n);\n";
...
int main(const int argc, char* argv[])
{
  ...
  static struct oak_test_t tests[] = {
    OAK_TEST_ENTRY(StdlibLexerTokenizeFieldAccess),
    OAK_TEST_ENTRY(StdlibLexerArrayReturnType),
  };
