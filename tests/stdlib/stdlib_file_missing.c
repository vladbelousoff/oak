...
/* Opening a non-existent path is a runtime error from the native 'open'. */
OAK_TEST_DECL(StdlibFileMissingPathRuntimeError)
{
  const char* source =
  ...
  oak_parse(lexer, OAK_NODE_PROGRAM, &pr, oak_mem_allocator());
