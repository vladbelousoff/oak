#include "oak_count_of.h"
#include "oak_test_pipeline.h"

#define INTERFACE_SHAPE \
  "interface IShape {\n" \
  "  fn area(self) -> number;\n" \
  "  fn describe(self) -> string;\n" \
  "}\n"

#define RECORD_CIRCLE \
  "record Circle { radius : number; }\n" \
  "fn Circle.area(self) -> number { return self.radius * self.radius; }\n" \
  "fn Circle.describe(self) -> string { return 'circle'; }\n"

/* =========================================================================
 * Valid programs that must compile and run
 * ========================================================================= */

OAK_TEST_DECL(InterfaceConformanceOk)
{
  return expect_ok(
      INTERFACE_SHAPE
      RECORD_CIRCLE
      "fn use_shape(s: IShape) { print(s.area()); }\n"
      "let c = new Circle { radius: 3 };\n"
      "use_shape(c);\n");
}

OAK_TEST_DECL(InterfaceVirtualCallOk)
{
  return expect_ok(
      INTERFACE_SHAPE
      RECORD_CIRCLE
      "fn use_shape(s: IShape) { print(s.describe()); print(s.area()); }\n"
      "let c = new Circle { radius: 2 };\n"
      "use_shape(c);\n");
}

OAK_TEST_DECL(InterfacePassAlreadyInterfaceObjectOk)
{
  return expect_ok(
      INTERFACE_SHAPE
      RECORD_CIRCLE
      "fn inner(s: IShape) { print(s.area()); }\n"
      "fn outer(s: IShape) { inner(s); }\n"
      "let c = new Circle { radius: 1 };\n"
      "outer(c);\n");
}

OAK_TEST_DECL(InterfaceDefaultMethodBodyOk)
{
  return expect_ok(
      "interface IGreet {\n"
      "  fn name(self) -> string;\n"
      "  fn greet(self) -> string { return 'hello ' + self.name(); }\n"
      "}\n"
      "record Person { n : string; }\n"
      "fn Person.name(self) -> string { return self.n; }\n"
      "fn Person.greet(self) -> string { return 'hello ' + self.n; }\n"
      "let p = new Person { n: 'world' };\n"
      "print(p.greet());\n");
}

/* =========================================================================
 * Conformance errors
 * ========================================================================= */

OAK_TEST_DECL(MissingInterfaceMethodRejected)
{
  return expect_compile_error(
      INTERFACE_SHAPE
      "record Bare { x : number; }\n"
      "fn use_shape(s: IShape) { print(s.area()); }\n"
      "let b = new Bare { x: 1 };\n"
      "use_shape(b);\n");
}

OAK_TEST_DECL(WrongReturnTypeRejected)
{
  return expect_compile_error(
      "interface IShape { fn area(self) -> number; }\n"
      "record Bad;\n"
      "fn Bad.area(self) -> string { return 'oops'; }\n"
      "fn use_shape(s: IShape) { print(s.area()); }\n"
      "let b = new Bad {};\n"
      "use_shape(b);\n");
}

OAK_TEST_DECL(WrongParamTypeRejected)
{
  return expect_compile_error(
      "interface IScalable { fn scale(self, factor: number) -> number; }\n"
      "record Bad { v : number; }\n"
      "fn Bad.scale(self, factor: string) -> number { return self.v; }\n"
      "fn use_it(s: IScalable) { print(s.scale(2)); }\n"
      "let b = new Bad { v: 1 };\n"
      "use_it(b);\n");
}

OAK_TEST_DECL(WrongArityRejected)
{
  return expect_compile_error(
      "interface IShape { fn area(self) -> number; }\n"
      "record Bad;\n"
      "fn Bad.area(self, extra: number) -> number { return extra; }\n"
      "fn use_shape(s: IShape) { print(s.area()); }\n"
      "let b = new Bad {};\n"
      "use_shape(b);\n");
}

/* =========================================================================
 * Dispatch errors
 * ========================================================================= */

OAK_TEST_DECL(InterfaceMethodNotOnInterfaceRejected)
{
  return expect_compile_error(
      INTERFACE_SHAPE
      RECORD_CIRCLE
      "fn use_shape(s: IShape) { print(s.missing()); }\n"
      "let c = new Circle { radius: 1 };\n"
      "use_shape(c);\n");
}

OAK_TEST_DECL(VirtualCallWrongArgTypeRejected)
{
  return expect_compile_error(
      "interface IScalable { fn scale(self, factor: number) -> number; }\n"
      "record Box { size : number; }\n"
      "fn Box.scale(self, factor: number) -> number { return self.size * factor; }\n"
      "fn use_it(s: IScalable) { print(s.scale('bad')); }\n"
      "let b = new Box { size: 5 };\n"
      "use_it(b);\n");
}

OAK_TEST_DECL(VirtualCallTooFewArgsRejected)
{
  return expect_compile_error(
      "interface IScalable { fn scale(self, factor: number) -> number; }\n"
      "record Box { size : number; }\n"
      "fn Box.scale(self, factor: number) -> number { return self.size * factor; }\n"
      "fn use_it(s: IScalable) { print(s.scale()); }\n"
      "let b = new Box { size: 5 };\n"
      "use_it(b);\n");
}

OAK_TEST_DECL(VirtualCallTooManyArgsRejected)
{
  return expect_compile_error(
      "interface IScalable { fn scale(self, factor: number) -> number; }\n"
      "record Box { size : number; }\n"
      "fn Box.scale(self, factor: number) -> number { return self.size * factor; }\n"
      "fn use_it(s: IScalable) { print(s.scale(1, 2)); }\n"
      "let b = new Box { size: 5 };\n"
      "use_it(b);\n");
}

/* =========================================================================
 * Duplicate / malformed declarations
 * ========================================================================= */

OAK_TEST_DECL(DuplicateInterfaceRejected)
{
  return expect_compile_error(
      "interface IShape { fn area(self) -> number; }\n"
      "interface IShape { fn area(self) -> number; }\n");
}

OAK_TEST_DECL(InterfaceNameMustStartWithIRejected)
{
  return expect_compile_error(
      "interface Shape { fn area(self) -> number; }\n");
}

/* =========================================================================
 * Interface arrays
 * ========================================================================= */

OAK_TEST_DECL(InterfaceArrayPushAndIterateOk)
{
  return expect_ok(
      INTERFACE_SHAPE
      RECORD_CIRCLE
      "record Rect { w : number; h : number; }\n"
      "fn Rect.area(self) -> number { return self.w * self.h; }\n"
      "fn Rect.describe(self) -> string { return 'rect'; }\n"
      "let mut shapes = new IShape[];\n"
      "shapes.push(new Circle { radius: 3 });\n"
      "shapes.push(new Rect { w: 2, h: 5 });\n"
      "let mut total = 0;\n"
      "for s in shapes { total += s.area(); }\n"
      "print(total);\n");
}

OAK_TEST_DECL(InterfaceArrayIndexOk)
{
  return expect_ok(
      INTERFACE_SHAPE
      RECORD_CIRCLE
      "let mut shapes = new IShape[];\n"
      "shapes.push(new Circle { radius: 4 });\n"
      "print(shapes[0].area());\n");
}

OAK_TEST_DECL(InterfaceArrayPushNonConformingRejected)
{
  return expect_compile_error(
      INTERFACE_SHAPE
      "record Plain { x : number; }\n"
      "let mut shapes = new IShape[];\n"
      "shapes.push(new Plain { x: 1 });\n");
}

OAK_TEST_DECL(MethodCoercionInMethodCallOk)
{
  return expect_ok(
      INTERFACE_SHAPE
      RECORD_CIRCLE
      "record Renderer;\n"
      "fn Renderer.render(self, s: IShape) { print(s.area()); }\n"
      "let r = new Renderer {};\n"
      "let c = new Circle { radius: 4 };\n"
      "r.render(c);\n");
}

/* Current behavior: a default method body does NOT auto-satisfy conformance.
 * A record must still explicitly implement every interface method, even ones with
 * a default body. This pins that invariant. If defaults
 * are ever auto-filled into vtables, update this test deliberately. */
OAK_TEST_DECL(InterfaceDefaultMethodDoesNotSatisfyConformance)
{
  return expect_compile_error(
      "interface IGreet {\n"
      "  fn name(self) -> string;\n"
      "  fn greet(self) -> string { return 'hi ' + self.name(); }\n"
      "}\n"
      "record Person { n : string; }\n"
      "fn Person.name(self) -> string { return self.n; }\n"
      /* greet() is intentionally not implemented */
      "fn use_greet(g : IGreet) { print(g.greet()); }\n"
      "let p = new Person { n: 'ada' };\n"
      "use_greet(p);\n");
}

int main(const int argc, char* argv[])
{
  (void)argc;
  (void)argv;
  static oak_test_t tests[] = {
    OAK_TEST_ENTRY(InterfaceConformanceOk),
    OAK_TEST_ENTRY(InterfaceVirtualCallOk),
    OAK_TEST_ENTRY(InterfacePassAlreadyInterfaceObjectOk),
    OAK_TEST_ENTRY(InterfaceDefaultMethodBodyOk),
    OAK_TEST_ENTRY(MissingInterfaceMethodRejected),
    OAK_TEST_ENTRY(WrongReturnTypeRejected),
    OAK_TEST_ENTRY(WrongParamTypeRejected),
    OAK_TEST_ENTRY(WrongArityRejected),
    OAK_TEST_ENTRY(InterfaceMethodNotOnInterfaceRejected),
    OAK_TEST_ENTRY(VirtualCallWrongArgTypeRejected),
    OAK_TEST_ENTRY(VirtualCallTooFewArgsRejected),
    OAK_TEST_ENTRY(VirtualCallTooManyArgsRejected),
    OAK_TEST_ENTRY(DuplicateInterfaceRejected),
    OAK_TEST_ENTRY(InterfaceNameMustStartWithIRejected),
    OAK_TEST_ENTRY(InterfaceArrayPushAndIterateOk),
    OAK_TEST_ENTRY(InterfaceArrayIndexOk),
    OAK_TEST_ENTRY(InterfaceArrayPushNonConformingRejected),
    OAK_TEST_ENTRY(MethodCoercionInMethodCallOk),
    OAK_TEST_ENTRY(InterfaceDefaultMethodDoesNotSatisfyConformance),
  };
  return oak_test_run(tests, (int)oak_count_of(tests));
}
