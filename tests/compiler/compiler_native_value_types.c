                                            ...
                                            oak_compile_options_t* opts)
{
  oak_lexer_result_t* lex =
      oak_lexer_tokenize(source, oak_test_allocator());
...
static oak_fn_call_result_t make_handle_impl(oak_native_ctx_t* ctx,
...
static oak_fn_call_result_t handle_id_impl(oak_native_ctx_t* ctx,
                                               const oak_value_t* args,
                                               int argc,
                                               oak_value_t* out_result)
{
 ...
 * constructor. */
static void bind_handle(oak_compile_options_t* opts)
{
  oak_bind_type_t* h =
...
{
  const oak_value_t a = oak_native_value_new((void*)(intptr_t)7);
  const oak_value_t b = oak_native_value_new((void*)(intptr_t)7);
  const oak_value_t c = oak_native_value_new((void*)(intptr_t)9);
...
static void bind_token(oak_compile_options_t* opts)
{
  oak_bind_type_t* t =
...
OAK_TEST_DECL(ValueTypeSameTypeEqualityOk)
{
  ...
  oak_compile_options_init(&opts, oak_test_allocator());
  bind_handle(&opts);
  const oak_test_status_t s = run_ok(
      "let a = make_handle();\nlet b = make_handle();\nprint(a == b);\n",
      &opts);
  oak_compile_options_free(&opts);
  return s;
