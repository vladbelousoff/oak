  ...
  oak_list_for_each(pos, &node->children)
  {
    ...
    if (oak_type_is_known(&et) && !oak_type_equal(&elem_ty, &et))
    {
      oak_compiler_error_at(c,
                            ...
                            "(expected '%s', got '%s')",
                            oc_type_full_name(c, elem_ty),
                            oc_type_full_name(c, et));
      return;
    }
                                      ...
                                      const struct oak_ast_node_t* node)
{
  ...
  oak_list_for_each(pos, &more->children)
  {
    ...
    oc_infer_type(c, entry->lhs, &kt);
    oc_infer_type(c, entry->rhs, &vt);
