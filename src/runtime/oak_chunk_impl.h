...
const oak_op_info_t* oak_op_get_info(u8 op);
...
/* oak_chunk_t and oak_chunk_disassemble are declared in the public oak_chunk.h. */
struct oak_chunk
{
...
void oak_chunk_enable_debug(oak_chunk_t* chunk,
                                    const char* source_name);

/* Intern a field-name layout. Returns a stable id >= 0, or -1 on failure. */
int oak_chunk_add_field_layout(oak_chunk_t* chunk,
                                       int field_count,
...
void oak_chunk_add_debug_local(oak_chunk_t* chunk,
                                       int slot,
                                       const char* name);
void oak_chunk_end_debug_local(oak_chunk_t* chunk, int slot);
