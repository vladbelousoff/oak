...
{
  if (!symbol || !symbol->name || symbol->name_len <= 0)
    return null;
  if (oak_symbol_registry_find(registry, symbol->name, symbol->name_len))
    return null;
  oak_assert(oak_dynarr_push(&registry->symbols, symbol));
  const int index = oak_dynarr_count(registry->symbols) - 1;
  oak_htable_insert(
      &registry->by_name, symbol->name, symbol->name_len, index);
...
{
  const int index = oak_htable_get(&registry->by_name, name, len);
  if (index < 0)
    return null;
  return &registry->symbols[index];
}
