...
struct oak_compiler_t
{
  ...
  int stack_depth;
  int had_error;
  struct oak_loop_ctx_t* current_loop;
};
...
static void
emit_byte(const struct oak_compiler_t* c, const uint8_t byte, const int line)
{
  oak_chunk_write(c->chunk, byte, line);
}

static void emit_op(struct oak_compiler_t* c, const uint8_t op, const int line)
{
  emit_byte(c, op, line);
...
static uint8_t make_constant(struct oak_compiler_t* c,
...
static size_t
emit_jump(struct oak_compiler_t* c, const uint8_t op, const int line)
{
  emit_op(c, op, line);
...
static void
emit_loop(struct oak_compiler_t* c, const size_t loop_start, const int line)
{
...
static void emit_pops(struct oak_compiler_t* c, int count, const int line)
{
  while (count-- > 0)
    emit_op(c, OAK_OP_POP, line);
}

static void emit_loop_jump(struct oak_compiler_t* c,
                           size_t* jumps,
                           int* count,
...
static int resolve_local(const struct oak_compiler_t* c,
                         const char* name,
                         const size_t length)
{
...
static int is_local_mutable(const struct oak_compiler_t* c,
                            const char* name,
                            const size_t length)
{
  for (int i = c->local_count - 1; i >= 0; --i)
...
static size_t list_length(const struct oak_ast_node_t* node)
{
  return oak_list_length(&node->children);
}

static uint8_t node_kind_to_op(const enum oak_node_kind_t kind)
{
  switch (kind)
  {
                         ...
                         const struct oak_ast_node_t* node)
{
  ...
  switch (node->kind)
  {
    ...
    case OAK_NODE_KIND_STRING:
    {
      ...
      struct oak_obj_string_t* str = oak_make_string(chars, len);
    ...
    case OAK_NODE_KIND_IDENT:
    {
      ...
      emit_op_arg(
          c, OAK_OP_GET_LOCAL, (uint8_t)slot, oak_token_line(node->token));
      break;
    }
    case OAK_NODE_KIND_BINARY_ADD:
    case OAK_NODE_KIND_BINARY_SUB:
    ...
    case OAK_NODE_KIND_STMT_ASSIGNMENT:
    {
      ...
      {
        compiler_error_at(
            c, lhs->token, "assignment target must be a variable");
        return;
      }

      const int slot = resolve_local(
          c, oak_token_buf(lhs->token), oak_token_size(lhs->token));
      if (slot < 0)
      {
        compiler_error_at(c,
                          lhs->token,
                          "undefined variable '%.*s'",
                          oak_token_size(lhs->token),
                          oak_token_buf(lhs->token));
        return;
      }
    ...
    case OAK_NODE_KIND_STMT_MOD_ASSIGN:
    {
      ...
      {
        compiler_error_at(
            c, lhs->token, "compound assignment target must be a variable");
        return;
      }
    ...
    case OAK_NODE_KIND_STMT_CONTINUE:
    {
      ...
      struct oak_loop_ctx_t* loop = c->current_loop;
      emit_loop_jump(c,
