...
/* block_size 0 selects OAK_ARENA_DEFAULT_BLOCK_SIZE. Safe to call again after destroy. */
void oak_arena_init(struct oak_arena_t* arena, usize block_size);

/* Returns zero-filled storage, aligned to 2 * sizeof(usize). Null on allocation failure. */
